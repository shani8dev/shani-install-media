...
per-build and **shipped inside the base image** (`scripts/build-base-image.sh:100`)
...
- **MOK private key in base image.** `scripts/build-base-image.sh:100` installs
...
- **Unencrypted signing key.** `keys/create-gpg-keys.sh:79` uses `%no-protection`
  — the GPG signing key is stored unencrypted.
- **SSH deploy key without passphrase.** `keys/create-ssh-keys.sh:127,134` sets
  `SSH_PASSPHRASE=""` — the deploy SSH key has no passphrase.
