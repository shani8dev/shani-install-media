...
Commands:
  ...
  iso-only       Download base image from R2, build ISO + repack + upload iso (requires -p <profile>)
...
Options:
  ...
  [mode]          Upload mode: 'image' (default), 'iso', or 'all' (for upload command)
  --from-r2       Download base image from R2 instead of requiring a local build (iso command)

Examples:
  $(basename "$0") image -p gnome
  $(basename "$0") flatpak -p gnome
  $(basename "$0") snap -p gnome
  $(basename "$0") iso -p gnome --from-r2           # Build ISO using latest image from R2
  $(basename "$0") iso-only -p gnome                # Download from R2, build ISO, repack, upload
...
# ---------------------------------------------------------------------------
# Helper: extract profile from remaining args (used by all/full/iso-only)
# ---------------------------------------------------------------------------
_get_profile() {
  local _prev=""
  local _profile=""
...
case "$COMMAND" in
  ...
  full)
    ...
    ;;
  iso-only)
    # Runs: iso (--from-r2, with local flatpak/snap if available) → repack → upload iso
    # Useful when the base image was already built and uploaded in a prior run.
    _ISO_PROFILE="$(_get_profile "$@")"
    [[ -z "$_ISO_PROFILE" ]] && die "Profile (-p) is required for the 'iso-only' command."

    ./scripts/build-iso.sh "$@" --from-r2
    ./scripts/repack-iso.sh "$@"
    ./scripts/upload.sh "$@" iso
    ;;
  publish)
    ./scripts/release.sh "$@"
    ./scripts/upload.sh "$@"
