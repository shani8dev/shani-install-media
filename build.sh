...
Commands:
  ...
  test           Install/boot/update/rollback a built image on loop-mounted disks (see test-env/)
  all            image → release latest → upload image
  full           image → flatpak → snap → iso → repack → release latest → upload all
...
Examples:
  ...
  $(basename "$0") test disk
  $(basename "$0") test bootstrap -p plasma
  $(basename "$0") test cycle -p plasma
EOF
  exit 1
}
