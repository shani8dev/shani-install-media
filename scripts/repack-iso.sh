...
[[ -n "$OUTPUT_ISO" ]] || die "No ISO found to repack."
...
osirrox -indev "$OUTPUT_ISO" -extract_boot_images "${REPACK_DIR}/" \
  ...
  -extract /shellx64.efi "${REPACK_DIR}/shellx64.efi" || die "EFI extraction failed"
...
[[ -f "$eltorito_img" ]] || die "Eltorito image not found."
...
cp "${mount_dir}/${OS_NAME}/boot/x86_64/vmlinuz-linux" "${REPACK_DIR}/vmlinuz-linux" || die "Copy vmlinuz-linux failed"
...
cp "${TEMP_DIR}/${PROFILE}/x86_64/airootfs/usr/share/shim-signed/shimx64.efi" "${REPACK_DIR}/BOOTx64.EFI" || die "Failed to copy shimx64.efi"
cp "${TEMP_DIR}/${PROFILE}/x86_64/airootfs/usr/share/shim-signed/mmx64.efi" "${REPACK_DIR}/" || die "Failed to copy mmx64.efi"
...
for file in grubx64.efi shellx64.efi vmlinuz-linux; do
  sbsign --key "${MOK_DIR}/MOK.key" --cert "${MOK_DIR}/MOK.crt" --output "${REPACK_DIR}/${file}" "${REPACK_DIR}/${file}" || die "Signing failed for ${file}"
done
...
rm -f "$final_iso" || die "Failed to remove old signed ISO"
...
xorriso -indev "$OUTPUT_ISO" -outdev "$final_iso" \
  ...
  -boot_image any replay -append_partition 2 0xef "$eltorito_img" || die "xorriso repack failed"

pushd "${OUTPUT_SUBDIR}" > /dev/null
sha256sum "$(basename "$final_iso")" > "$(basename "$final_iso").sha256" || die "Checksum generation failed"
...
gpg --homedir "$GNUPGHOME" \
    ...
    "${final_iso}" || die "GPG signing of ISO failed"
...
mktorrent \
  ...
  -l 22 \
  -o "${TORRENT_FILE}" \
  "${final_iso}" || die "mktorrent failed"
